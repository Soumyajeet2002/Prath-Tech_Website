...
// const API_URL = window.PRATHTECH_API_URL || "http://192.168.0.24:5000";
const API_URL = window.PRATHTECH_API_URL || "/api";
// const API_URL = window.PRATHTECH_API_URL || "/prathtech-website";
// const BOT_AVATAR =
//   "https://gov.silicontechlab.com/prathtech_website/images/logo.svg";
const BOT_AVATAR =
  "./images/blue-logo-PTPL.svg";
...
async function startChatSession() {
  return safeFetch(`${API_URL}/chat/session`, {
    method: "POST",
...
async function getAvailableSlots() {
  const res = await fetch(`${API_URL}/bookings/slots`);
  const json = await res.json();
...
async function getChatHistory(sessionId) {
  return safeFetch(`${API_URL}/chat/history/${sessionId}`);
}
...
function addMessage(msgData, withOptions = false) {
  ...
  // scrollToBottom();
   ...
   setTimeout(() => {
    ...
    const bots = document.querySelectorAll('.msg-row.bot');

    const latestBot = bots[bots.length - 1];

    if (latestBot) {
      latestBot.scrollIntoView({
        behavior: "smooth",
        block: "start"
      });
    }
